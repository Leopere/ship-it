...
After a successful push, ship-it invokes `deploy-it` when the pushed revision contains `.deploy-it.json`. The repository command owns the destination and live acceptance check. Repositories without this file still ship normally. If production delivery is requested, finish implementing the repository deployment command and its live acceptance checks before ending the turn. The native hook runs that command after the push. Use existing project configuration and the authorized destination to prepare it.

Do not require a clean worktree before shipping or deployment. Shipping stages all changes, commits when needed, and pushes everything Git stages without content selection or approval prompts. Deployment uses the exact pushed commit in a private snapshot. Local worktree and index changes do not block deployment or enter that snapshot. Pending edits belong to the native shipping flow; they are not a deployment blocker.
